...
public class ApiDetailExplorer {
    ...
    public String GetAptApi(String aptUrl, int pageNoOrMangeNo, boolean isPageNo) {
        ...
        try {
            ...
            if (isPageNo) {
                urlBuilder.append("&").append(URLEncoder.encode("startmonth", StandardCharsets.UTF_8)).append("=").append(URLEncoder.encode("202101", StandardCharsets.UTF_8)); /*월 단위 모집공고일 (검색시작월)*/
    ...
    public static void main(String[] args) {
        ...
        for (Integer number : numbers) {//주택관리번호 리스트 크기만큼 반복
            ...
            for (com.hanium.chungyakpassback.dto.아파트분양정보1Dto 아파트분양정보1Dto : 아파트분양정보1DtoList) {
                아파트분양정보1Dto.PrintDate();
            }
        }
        }
    }
